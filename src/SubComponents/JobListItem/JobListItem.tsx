...
function JobListItem({
  ...
  const overallProgress = isError
    ? "-1"
    : (
        subjobs.reduce((acc, el) => acc + el.progress, 0) / subjobs.length
      ).toFixed(1);
  ...
  const getColorStyle = (_progress: number) =>
    _progress == -1
      ? ({
          "--main-color": "color-mix(in srgb, var(--tertiary-color) 60%, red)",
        } as CSSProperties)
      : _progress == 0
        ...
        : _progress == 100
          ? ({
              "--main-color":
                "color-mix(in srgb, var(--secondary-color) 60%, var(--background-color))",
            } as CSSProperties)
  ...
  return (
    <>
      <div
        ...
        style={getColorStyle(Number(overallProgress))}
      >
        <button
          className={"JobListItem__AccordionButton"}
          onClick={() => setIsOpen(!isOpen)}
        >
          <div
            className="JobListItem__ProgressBar"
            style={getProgressStyle(Number(overallProgress))}
          ></div>
        ...
        <div className="JobListItem__SubJobList">
          {subjobs.map((subjob, index) => (
            <div
              className={isOpen ? "SubJobListItem show" : "SubJobListItem"}
              style={getColorStyle(subjob.progress)}
              key={index}
            >
              <div
                className="SubJobListItem__ProgressBar"
                style={getProgressStyle(subjob.progress)}
              ></div>
